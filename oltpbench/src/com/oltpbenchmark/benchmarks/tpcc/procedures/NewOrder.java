...
public class NewOrder extends TPCCProcedure {
    ...
    public final SQLStmt stmtGetCustWhseSQL = new SQLStmt("SELECT c_discount, c_last, c_credit, w_tax"
							  + "  FROM " + TPCCConstants.TABLENAME_CUSTOMER + ", " + TPCCConstants.TABLENAME_WAREHOUSE
							  + " WHERE w_id = ? AND c_w_id = ?"
							  + " AND c_d_id = ? AND c_id = ?");
    
    public final SQLStmt stmtGetDistSQL = new SQLStmt("SELECT d_next_o_id, d_tax FROM " + TPCCConstants.TABLENAME_DISTRICT
