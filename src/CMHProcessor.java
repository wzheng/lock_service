 ...
 * Designed to be set up on a single "process" upon creation
 */
public class CMHProcessor {
	 ...
	 * @param message String JSON message to be processed
	 */
	...
	public boolean detectDeadlock(int initiatorTID, int fromTID, int thisTID){
		...
		// if it is waiting for another resource already, forward the next message in the chain
		if (waitingForTIDs.size() > 0){
			for (int nextTID : waitingForTIDs){
				CMHMessage nextMessage = new CMHMessage(initiatorTID, thisTID, nextTID);
