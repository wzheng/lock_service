...
to be able to detect them efficiently so that the database can continue to handle transactions. The transactions involved in a distributed deadlock may be located at multiple servers, so a distributed version of deadlock detection is necessary.
...
Currently, two-phase commit does not have a recovery process built into it. This means that the database will not be able to recover if a failure occurs at any server or across the network. The two-phase commit protocol used in this project could be extended to perform logging to enable fault tolerance.
